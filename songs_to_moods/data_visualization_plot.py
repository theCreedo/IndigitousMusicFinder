	...
	if not os.path.isdir(path):
		...
		if ext == 'txt':
			try:
				...
				# EDIT FILEPATH BASED ON BASEPATH
				with open('./Sorted_Tones/' + filename + '.txt', 'r') as data_file:
					for line in data_file:
						...
						if count % 2 == 0:
							...
							plt.xlim(0,100)
							if imgcount < 10:
								plt.savefig('./images/' + tonemood[0] + '/00' + str(imgcount) + '_' + name + '.png')
							elif imgcount < 100:
								plt.savefig('./images/' + tonemood[0] + '/0' + str(imgcount) + '_' + name + '.png')
							else:
								plt.savefig('./images/' + tonemood[0] + '/' + str(imgcount) + '_' + name + '.png')
							plt.clf()
							imgcount += 1
						count+=1
