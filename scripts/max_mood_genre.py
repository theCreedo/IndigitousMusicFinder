...
for fname in os.listdir(basepath + '../data/json/'):
    ...
    if not os.path.isdir(path):
        ...
        if ext == 'json':
            outfile = open('./../data/txt/exception_dummy.txt', 'w')
            outfile.write('this is for exception catching purposes')
            try:
                data = ''
                # Get the whole file and load as json - Change Filenames based on basepath
                with open('../data/json/' + filename + '.json', 'r') as data_file:
                ...
                outfile = open('./../data/txt/tone_analyzed_songs/' + entire_tone_word + '/' + filename + '.txt', 'w')
